...
function widgets.mediaGallery(mediaType, customTileSize, customPageItems)
  ...
  -- Dynamically determine sorting fields for this specific media type
  local sortFields = {}
  local seenFields = {}
  local function addSortField(f)
      if f and not seenFields[f] then
  ...
  addSortField("title")
  for _, f in ipairs(currentCfg[3] or {}) do addSortField(f) end
  addSortField(currentCfg[4] or "score")
  for _, f in ipairs(currentCfg[6] or {}) do addSortField(f) end

  local sortOptionsHtml = ""
  for _, f in ipairs(sortFields) do
      -- Safe capitalization without relying on global string table
...
--      items = query[[from index.tag(page) where type == queryTag order by title]]
  ...
  html = html .. [[
    <div class="media-controls">
      ...
      <div class="media-sort-container">
        <label class="media-sort-label">Order by: </label>
        <select class="media-sort-select">
  ]] .. sortOptionsHtml .. [[
        </select>
        <div class="media-sort-buttons">
            <button class="sort-btn active" data-dir="asc" title="Sort A-Z">A-Z</button>
            <button class="sort-btn" data-dir="desc" title="Sort Z-A">Z-A</button>
        </div>
      </div>
  ...
  for _, item in ipairs(items) do
    ...
    -- Gather Sort Data attributes dynamically based on allowed fields for this type
    local sortData = ""
    for _, f in ipairs(sortFields) do
        local val = item[f] or ""
        if f == "score" or f == "imdb_rating" then val = score end
        if f == "title" then val = title end
        if type(val) == "table" then val = table.concat(val, ", ") end
        sortData = sortData .. " data-sort-" .. f .. "='" .. tostring(val):gsub("'", "&apos;") .. "'"
    end
    ...
    html = html .. 
      "<a class='media-card' href='" .. link .. "' data-ref='" .. link .. "' data-filter='" .. filterContent .. "'" .. sortData .. ">" ..
  ...
  scriptEl.innerHTML = [[
    (function() {
        ...
        const init = () => {
            ...
            const sortSelect = root.querySelector(".media-sort-select");
            ...
            const updateDisplay = () => {
                ...
                filteredCards.sort((a, b) => {
                    let valA = a.getAttribute("data-sort-" + sortField) || "";
                ...
                filteredCards.slice(start, end).forEach((card, idx) => {
                    ...
                    card.style.order = idx; // Maintain sorted order in flex/grid
                });
...
```space-style
html{
  --embed-background: var(--modal-background-color);
...
.media-filter-label, .media-sort-label {
  font-weight: 600;
  font-size: 0.9em;
